...
if __name__ == "__main__":  
    ...
    anom_matrices = np.zeros((n_anomalies, MAX_SIZE,MAX_SIZE))
    ...
    train_matrices = np.zeros((n_train, MAX_SIZE,MAX_SIZE))
    train_features = np.zeros((n_train,MAX_SIZE,NODE_FEATURES))

    #Stores the validation set
    valid_matrices = np.zeros((n_valid, MAX_SIZE,MAX_SIZE))
    valid_features = np.zeros((n_valid,MAX_SIZE,NODE_FEATURES))

    #Stores the test set 
    test_matrices = np.zeros((n_test, MAX_SIZE,MAX_SIZE))
    ...
    if anomaly_type == 'Clique':
        G,anomaly_indices = AddCliques(G,num_cliques=9,clique_size=12)
        anomaly_nodes = anomaly_indices

    elif anomaly_type == 'Structure-Swap':
        folder2 = sys.argv[5]
        H = nx.read_edgelist(folder2 + '_edges.txt',create_using=nx.Graph(),nodetype=str)
        anom_matrices,anom_features,anom_sizes,anom_degrees,anom_triangles = \
    ...
    for i,start_node in enumerate(random_nodes):
        ...
        adjacency_tensor,X = generateFeatures(A,bfs_order,node_feature_dict,EDGE_FEATURES,NODE_FEATURES)
    ...
    '''
    Optional - apply a minimum threshold on neighborhood size 
    thresh = 3
    #Discards all training samples that are too small 
    train_matrices = train_matrices[train_sizes > thresh]
    train_features = train_features[train_sizes > thresh]
    total_degrees = total_degrees[train_sizes > thresh]
    total_triangles = total_triangles[train_sizes > thresh]
    train_sizes = train_sizes[train_sizes > thresh]

    test_matrices = test_matrices[test_sizes > thresh]
    test_features = test_features[test_sizes > thresh]
    test_sizes = test_sizes[test_sizes > thresh]

    anom_matrices = anom_matrices[anom_sizes > thresh]
    anom_features = anom_features[anom_sizes > thresh]
    extra_degrees = anom_degrees[anom_sizes > thresh]
    anom_triangles = anom_triangles[anom_sizes > thresh]
    anom_sizes = anom_sizes[anom_sizes > thresh]
    '''
