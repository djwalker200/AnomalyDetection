...
def generateFeatures(SubG,node_order,feature_dict,EDGE_FEATURES,NODE_FEATURES):
    ...
    if EDGE_FEATURES > 1:
        ...
        #Stores the adjacency tensor
        adjacency_tensor = np.zeros((EDGE_FEATURES,num_nodes,num_nodes))
        for e in edges:
            idx1 = node_order.index(e[0])
            idx2 = node_order.index(e[1])
            edge_type = e[2]['weight']['weight']
            adjacency_tensor[edge_type,idx1,idx2] = 1



    #Should be used for standard edges
    elif EDGE_FEATURES == 1:
        #A = np.expand_dims(A,axis=0)
        #adjacency_tensor = np.concatenate((A,1-A),axis=0)
        adjacency_tensor = np.expand_dims(A,axis=0)
    else:
    ...
    for n in range(num_cliques):
        ...
        for i in range(clique_size):
            for j in range(i,clique_size):
                G.add_edge(clique_nodes[i],clique_nodes[j],weight=1)
...
def Scramble(G, feature_dict, N, NODE_FEATURES):
...
def GenerateSwapAnomalies(G,H,N,MAX_SIZE,NODE_FEATURES,feature_dict):
    ...
    anomaly_adj = np.zeros((N,1,MAX_SIZE,MAX_SIZE))
    degrees = []
    triangles = []
    for i,start_node in enumerate(random_nodes):
        H_start = H_random_nodes[i]

        SubG,bfs_order,__ = neighborhood_BFS(G,start_node,3,MAX_SIZE)


        n = len(bfs_order)
        SubH,__,__ = neighborhood_BFS(H,H_start,3,n)
