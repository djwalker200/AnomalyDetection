...
class RelationGraphConvolution(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        :param adj: (batch, EDGE_FEATURES + 1, N, N)
        ...
        output = torch.einsum('...ij, ...jh-> ...ih', adj, support)  # (batch, EDGE_FEATURES + 1, N, d)
...
class RGCN(nn.Module):
    def __init__(self, nfeat, nhid=128, nout=128, edge_dim=3, num_layers=3, dropout=0.0, normalization=False,gcn_bias=False):
        '''
        ...
        self.gc1 = RelationGraphConvolution(
            nfeat, nhid, edge_dim=self.edge_dim, aggregate='sum', use_relu=True, dropout=self.dropout, bias = gcn_bias)
        ...
        self.gc3 = RelationGraphConvolution(
            nhid, nout, edge_dim=self.edge_dim, aggregate='sum', use_relu=False, dropout=self.dropout, bias= gcn_bias)
        # if self.normalization
...
class MaskedGraphAF(nn.Module):
    def __init__(self, mask_node, mask_edge, index_select_edge, num_flow_layer, graph_size,
        ...
        self.rgcn = RGCN(num_node_type, nhid=self.hid_size, nout=self.emb_size, edge_dim=self.num_edge_type,
    ...
    def forward(self, x, adj, x_deq, adj_deq):
        ...
        :param adj_deq:  (batch, edge_num, EDGE_FEATURES + 1)
        :return:
        '''
        # inputs for RelGCNs
        batch_size = x.size(0)
        ...
        x_deq = x_deq.view(-1, self.num_node_type)  # (batch *N, NODE_FEATURES)
        adj_deq = adj_deq.view(-1, self.num_edge_type) # (batch*(repeat_num-N), EDGE_FEATURES) 
        ...
        x_deq = x_deq.view(batch_size, -1)  # (batch, N * NODE_FEATURES)
        adj_deq = adj_deq.view(batch_size, -1)  # (batch, (repeat_num-N) * EDGE_FEATURES+1)
    ...
    def reverse(self, x, adj, latent, mode, edge_index=None):
        '''
        Args:
            x: generated subgraph node features so far with shape (1, N, NODE_FEATURES), some part of the x is masked
            adj: generated subgraph adacency features so far with shape (1, EDGE_FEATURES + 1, N, N) some part of the adj is masked
            latent: sample latent vector with shape (1, NODE_FEATURES) (mode == 0) or (1, EDGE_FEATURES + 1) (mode == 1)
            mode: generation mode. if mode == 0, generate a new node, if mode == 1, generate a new edge
            edge_index [1, 2]

        Returns:
            out: generated node/edge features with shape (1, NODE_FEATURES) (mode == 0) or (1, EDGE_FEATURES + 1) , (mode == 1)
        '''
    ...
    def _get_embs_node(self, x, adj):
        """
        Args:
            x: current node feature matrix with shape (batch, N, NODE_FEATURES)
            adj: current adjacency feature matrix with shape (batch, EDGE_FEATURES + 1, N, N)
        Returns:
    ...
    def _get_embs(self, x, adj):
        '''
        :param x of shape (batch, N, NODE_FEATURES)
        :param adj of shape (batch, EDGE_FEATURES + 1, N, N)
        ...
        # inputs for RelGCNs
        batch_size = x.size(0)
        #adj = adj[:, :-1] # (batch, EDGE_FEATURES, N, N) 
        #TODO: check whether we have to use the 4-th slices(virtual bond) or not



        if self.args.cuda:
            x = torch.where(self.mask_node, x.unsqueeze(1).repeat(1, self.repeat_num, 1, 1), torch.zeros([1]).cuda())
            x = x.view(-1, self.graph_size, self.num_node_type)  # (batch*repeat_num, N, NODE_FEATURES)

            adj = torch.where(self.mask_edge, adj.unsqueeze(1).repeat(1, self.repeat_num, 1, 1, 1),torch.zeros([1]).cuda())
            adj = adj.view(-1, self.num_edge_type, self.graph_size, self.graph_size)  # (batch*repeat_num, EDGE_FEATURES, N, N) 
        else:
            x = torch.where(self.mask_node, x.unsqueeze(1).repeat(1, self.repeat_num, 1, 1), torch.zeros([1]))
            x = x.view(-1, self.graph_size, self.num_node_type)  # (batch*repeat_num, N, NODE_FEATURES)

            adj = torch.where(self.mask_edge, adj.unsqueeze(1).repeat(1, self.repeat_num, 1, 1, 1),torch.zeros([1]))
            adj = adj.view(-1, self.num_edge_type, self.graph_size, self.graph_size)  # (batch*repeat_num, EDGE_FEATURES, N, N) 
