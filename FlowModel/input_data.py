...
def generateFeatures(A,node_order,feature_dict,EDGE_FEATURES,NODE_FEATURES):
    ...
    if EDGE_FEATURES > 1:
        ...
        adjacency_tensor = np.zeros((NUM_EDGE_FEATURES,num_nodes,num_nodes))

        #For now, randomly assign edge features
        edge_features = np.random.randint(1,NUM_EDGE_FEATURES + 1,size=(num_nodes,num_nodes))
        edge_features = np.multiply(edge_features, A)

        #USE THIS FOR NOW
        for j in range(NUM_EDGE_FEATURES):
            current = adjacency_tensor[j]
            current[edge_features == j + 1] = 1

        A = 1 - A
        A = np.expand_dims(A,axis=0)
        adjacency_tensor = np.concatenate((adjacency_tensor,A),axis=0)
