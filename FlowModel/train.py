...
def read_molecules(folder,name):
...
class Trainer(object):
    def __init__(self,dataloader, validation_dataloader, data_config, args):
        ...
        self.edge_dim = self.data_config['edge_dim']
       
        
        self._model = GraphFlowModel(self.max_size, self.node_dim, self.edge_dim, self.args.edge_unroll, self.args)
    ...
    def train_epoch(self, epoch_cnt):
        ...
        for i_batch,batch_data in enumerate(self.valid_dataloader):
            ...
            inp_node_features = batch_data['node'] #(BATCH_SIZE, NUM_NODES, NODE_FEATURES)
            inp_adj_features = batch_data['adj'] #(BATCH_SIZE, EDGE_FEATURES, NUM_NODES, NUM_NODES)   
...
def find_distribution(dataloader,model,args):
    ...
    for i_batch,batch_data in enumerate(dataloader):
        ...
        if args.deq_type == 'random':
            ...
            out_z, out_logdet, ln_var = model(nodes,adj)
            loss = model.log_prob(out_z, out_logdet,aggregate_loss = False)
            
        #Updates the array of log-likelihoods
        if i_batch == 0:
            probs = loss.detach().cpu().numpy()
        else:
            probs = np.concatenate((probs,loss.detach().cpu().numpy()),axis=0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--edge_unroll', type=int, default=49, help='max edge to model for each node in bfs order.')
    ...
    train_node_features, train_adj_features, train_sizes, data_config  = read_molecules(args.path,'train')
    valid_node_features, valid_adj_features, valid_sizes, data_config = read_molecules(args.path,'valid')
    test_node_features,test_adj_features,test_sizes,data_config = read_molecules(args.path,'test')
    ...
    #Loads anomaly data
    anom_node_features,anom_adj_features,anom_sizes,data_config,__ = read_molecules(args.path,'extra')
    ...
    train_probs = find_distribution(train_dataloader,test_model,args)
    test_probs = find_distribution(test_dataloader,test_model,args)
    anom_probs = find_distribution(anom_dataloader,test_model,args)

    #Concatenates
    total_probs = np.concatenate((test_probs,anom_probs))

    #Sets limits for graphics
    min_x = np.min(total_probs) - 100
    max_x = np.max(total_probs) + 100
